        ...
        <b-card header-tag="h4" class="bg-primary-card weather-widget">
            <div class="row weather-data">
                <div class="col-lg-12 temperature">
                    <h2>
                        <i class="wi icon" v-if="weather.list" :class="('wi-owm-'+weather.list[0].weather[0].id)"></i>
                        <span class="pull-right" v-if="weather.list">{{weather.list[0].temp.day}}
                            <sup>
                                <sup>o</sup>
                            </sup>c
                            <br>
                            <span class="location">
                                <i class="fa fa-map-marker text-default" aria-hidden="true"></i>
                                {{weather.city.name}}, {{weather.city.country}}</span>
                        </span>
                    </h2>
                </div>
                <div class="weather-footer mt-5">
                    <div class="row text-center mt-1" v-if="weather.list">
                        <div class="col-md-2 col-4 popup" v-for="(day,index) in weather.list" v-if="index>0" :key="index">
                            <h5>{{getday(day.dt)}}</h5>
    ...
    export default {
        ...
        methods: {
            ...
            showdata() {
                ...
                axios.get("http://api.openweathermap.org/data/2.5/forecast/daily?" + getdata + "&appid=" + this.appid +
...
<style scoped>
    /*weather widget*/
    .weather-widget {
        background-image: url("~img/pages/weathernew.jpg");
