...
public   class WebUtils {
	...
	public static ResData _doGetAsByte(ShowapiRequest req) throws IOException {
		...
		try {
			...
			try {
				conn = getConnection(buildGetUrl(req.getUrl(), query), METHOD_GET, ctype, 
						req.getHeadMap(),req.getProxy(),req.isAllowRedirect());
			...
			try {
				...
				req.setResponseCode(conn.getResponseCode());
			} catch (IOException e) {
				ShowApiLogger.logCommError(e, req.getUrl(), req.getTextMap().get("showapi_app_id"),  req.getTextMap());
				throw e;
			}
	...
	public static ResData _doPostAsByte(ShowapiRequest req) throws IOException {
		...
		try {
			try {
				conn = getConnection(new URL(req.getUrl()), METHOD_POST, ctype, req.getHeadMap(),req.getProxy(),req.isAllowRedirect());
				conn.setConnectTimeout(req.getConnectTimeout());
				conn.setReadTimeout(req.getReadTimeout());
	...
	private static HttpURLConnection getConnection(URL url, String method, String ctype,
			Map<String, String> headerMap,Proxy proxy,boolean allowRedirect) throws IOException {
	...
	protected static byte[]  getResponseAsByte(HttpURLConnection conn ) throws IOException {
		...
		if (es == null) {
			if(conn.getContentEncoding()!=null&&conn.getContentEncoding().toLowerCase().equals("gzip")){
				...
				return readByteFromStream(conn.getInputStream() );
			}
		} else {
